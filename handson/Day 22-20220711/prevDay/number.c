...
void insert()
{
	char fname[]="num.dat";
        ...
        for(double i=0;i<MAX_WORKERS;i++)    
        {        
        	fwrite(&i,1,sizeof(double),out);    
        } 
        fclose(out);   
}
void read(double *array)
{
	char fname[]="num.dat";
        ...
        {        
        	fread((array + i),1,sizeof(double),in); 
        	//printf(" %lf,",array[i]);   
        } 
        fclose(in);   
}
void doSum(double *array)
{
...
void bulk_process(double* array)
{
...
void bulk_process_v2(double* array)
{
	thrd_t workers[MAX_WORKERS/BATCH_SIZE];
 		for(int i=0;i<MAX_WORKERS/BATCH_SIZE;i++)    
        {      
...
{    
		//printf("line 52\n");
		double* array = (double *)malloc(sizeof(double)*MAX_WORKERS);
		insert();
 		read(array);
 		//printf("line 56: %lf\n",array[MAX_WORKERS-1]);
 		int n=5;
